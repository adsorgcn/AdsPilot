        ...
        if check_health >/dev/null 2>&1; then
            ...
            if [[ -n "$ALERT_WEBHOOK" ]]; then
                curl -X POST -H 'Content-type: application/json' \
                    --data "{\"text\":\"🚨 Google-Monetize SaaS ($ENVIRONMENT) 健康检查失败\"}" \
...
main() {
    ...
    log_info "Google-Monetize SaaS 健康检查 - 环境: $ENVIRONMENT"
