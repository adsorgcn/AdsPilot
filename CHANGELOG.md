...
自测：T6-a 到 T6-d 先在 2.0.16 上跑出失败再修，修后全过；其余不变。

真实世界：CC 2026-09-26 核的结果：加上地区 ID 后建系列前的推荐能调通，那次谷歌没给推荐；已有系列的查询改成整个选后能过，0 行；报表的首页出价估计字段谷歌收下了，没退回，但账号近 7 天没有关键词数据，值没法核。本版改动后还要再核一轮。
