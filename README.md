...
截至 2026-09-26，版本 2.0.17。下面每一块都有代码、有使用方法、有自测，区别只在有没有碰过真实世界。
...
| 默认 SOUL | 完成 | 金额按美元定义、按账户币种折算；不写出价与预算常数，出价兜底与预算照谷歌推荐；自定义边界能被代码执行，换 SOUL 判断插件跟着换；阈值从 SOP 抄的，没被真数据校过 |
